...
static PyStructSequence_Field abi_info_fields[] = {
    {"threading", "Is free-threading enabled?"},
...
PyObject *
_PyAbiInfo_GetThreading(void)
{
    int res;
    PyObject *threading, *threading_support;
    threading = PyFrozenSet_New(NULL);
    if (threading == NULL) {
        goto error;
    }

#ifdef Py_GIL_DISABLED
    threading_support = PyUnicode_FromString("free-threading");
#else
    threading_support = PyUnicode_FromString("gil-enabled");
#endif
    if (threading_support == NULL) {
        goto error;
    }

    res = PySet_Add(threading, threading_support);
    Py_DECREF(threading_support);
    if (res < 0) {
        goto error;
    }
